...
const IPFront='192.168.1.104';
//Port number of FrontEnd Server
const PortFront='5000';
//In order to make request,We use SuperAgent (To send requst to Catalog Service + Order Service)
const superagent = require('superagent');
app.use(bp.json());
app.use(bp.urlencoded({ extended: true }));
//Start Catalog Sever at 5000 port also we print the statement when the server start up
app.listen(5000,()=>{
console.log("Catalog Server is listening at port 5000");
});
// Connecting  SQlite Database
const sqlite3 = require('sqlite3');
let db = new sqlite3.Database("./Catalog1.db" , (err) => {
    if(err) {
...
//Handle GET requst IPcatalog:5000/info/id + Notice Here We send thee id in the header
...
//Handle GET requst IPcatalog:5000/info + Notice Here We send thee id in the body
...
//Handle GET requst IPcatalog:5000/Search/topic + Notice Here We send thee topic in the header
