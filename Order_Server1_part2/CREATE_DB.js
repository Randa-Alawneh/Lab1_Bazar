...
const filepath = "./Order1.db";//Path of DATABASE
function connectToDatabase() { // Function to connect with database
  // if Order1.db was not found create it   
  if (fs.existsSync(filepath)) {
    return new sqlite3.Database(filepath);
  }
  
// if Order1.db was found then open it to create table 
  else {
    const db = new sqlite3.Database(filepath, (error) => {
      if (error) {
        return console.error(error.message);
      }
      createTable(db);// Function to create table in Order database
