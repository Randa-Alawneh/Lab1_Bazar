...
app.listen(3000,()=>{
console.log("Order2 Server is listening at port 3000");
});
let RC;
...
let db = new sqlite3.Database("./Order2.db" , (err) => {
    if(err) {
...
if (json.text=="The Book Avalaible In Stock"){
  ...
  const gmt = new Date(dateTime).toUTCString()  ;
   ...
   superagent.post("http://"+IPOrder1+":"+PortOrder1+"/ADD").send({ID:r,date:gmt}).then();
